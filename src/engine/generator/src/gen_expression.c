...
static KarExpressionResult kar_expression_result_none() {
...
static KarExpressionResult kar_expression_result_var(KarVartree* var) {
	KarExpressionResult result;
	result.type = var;
	result.value = NULL;
	return result;
...
static bool cast_type(KarExpressionResult* varFrom, KarExpressionResult* varTo, KarLLVMData *llvmData, KarVars *vars) {
	...
	} else if (varFrom->type == vars->standard.literalFloatType) {
...
static KarExpressionResult get_val_float(KarToken* token, KarString* moduleName, KarVars* vars, KarProjectErrorList* errors) {
	...
	KarExpressionResult result;
	result.type = vars->standard.literalFloatType;
...
static KarExpressionResult getCleanValue(KarExpressionResult res_clean, KarToken* token, KarLLVMData* llvmData, KarString* moduleName, KarVars* vars, KarProjectErrorList* errors) {
	...
	if (res_clean.type == vars->standard.boolType) {
		...
		result.value = LLVMBuildCall(llvmData->builder, llvmData->cleanBool, &res_clean.value, 1, "asdf");
	} else if (res_clean.type == vars->standard.decimalType) {
		result.type = vars->standard.uncleanDecimal;
		result.value = LLVMBuildCall(llvmData->builder, llvmData->cleanInteger64, &res_clean.value, 1, "asdf");
	} else if (res_clean.type == vars->standard.hexadecimalType) {
		result.type = vars->standard.uncleanHexadecimal;
		result.value = LLVMBuildCall(llvmData->builder, llvmData->cleanUnsigned64, &res_clean.value, 1, "asdf");
	} else if (res_clean.type == vars->standard.literalFloatType) {
		result.type = vars->standard.uncleanLiteralFloat;
		result.value = LLVMBuildCall(llvmData->builder, llvmData->cleanFloat64, &res_clean.value, 1, "asdf");
	} else if (res_clean.type == vars->standard.int8Type) {
		result.type = vars->standard.uncleanInt8;
		result.value = LLVMBuildCall(llvmData->builder, llvmData->cleanInteger8, &res_clean.value, 1, "asdf");
	} else if (res_clean.type == vars->standard.int16Type) {
		result.type = vars->standard.uncleanInt16;
		result.value = LLVMBuildCall(llvmData->builder, llvmData->cleanInteger16, &res_clean.value, 1, "asdf");
	} else if (res_clean.type == vars->standard.int32Type) {
		result.type = vars->standard.uncleanInt32;
		result.value = LLVMBuildCall(llvmData->builder, llvmData->cleanInteger32, &res_clean.value, 1, "asdf");
	} else if (res_clean.type == vars->standard.int64Type) {
		result.type = vars->standard.uncleanInt64;
		result.value = LLVMBuildCall(llvmData->builder, llvmData->cleanInteger64, &res_clean.value, 1, "asdf");
	} else if (res_clean.type == vars->standard.unsigned8Type) {
		result.type = vars->standard.uncleanUnsigned8;
		result.value = LLVMBuildCall(llvmData->builder, llvmData->cleanUnsigned8, &res_clean.value, 1, "asdf");
	} else if (res_clean.type == vars->standard.unsigned16Type) {
		result.type = vars->standard.uncleanUnsigned16;
		result.value = LLVMBuildCall(llvmData->builder, llvmData->cleanUnsigned16, &res_clean.value, 1, "asdf");
	} else if (res_clean.type == vars->standard.unsigned32Type) {
		result.type = vars->standard.uncleanUnsigned32;
		result.value = LLVMBuildCall(llvmData->builder, llvmData->cleanUnsigned32, &res_clean.value, 1, "asdf");
	} else if (res_clean.type == vars->standard.unsigned64Type) {
		result.type = vars->standard.uncleanUnsigned64;
		result.value = LLVMBuildCall(llvmData->builder, llvmData->cleanUnsigned64, &res_clean.value, 1, "asdf");
	} else if (res_clean.type == vars->standard.float32Type) {
		result.type = vars->standard.uncleanFloat32;
		result.value = LLVMBuildCall(llvmData->builder, llvmData->cleanFloat32, &res_clean.value, 1, "asdf");
	} else if (res_clean.type == vars->standard.float64Type) {
		result.type = vars->standard.uncleanFloat64;
		result.value = LLVMBuildCall(llvmData->builder, llvmData->cleanFloat64, &res_clean.value, 1, "asdf");
...
static KarExpressionResult get_sign_single_minus(KarToken* token, KarLLVMData* llvmData, KarString* moduleName, KarVars* vars, KarProjectErrorList* errors) {
	...
	if (res.type == vars->standard.literalFloatType ||
		res.type == vars->standard.float32Type ||
		res.type == vars->standard.float64Type
	) {
...
static KarExpressionResult get_sign_minus(KarToken* token, KarLLVMData* llvmData, KarString* moduleName, KarVars* vars, KarProjectErrorList* errors) {
	...
	if (leftRes.type == vars->standard.decimalType ||
		...
		KarExpressionResult res = kar_expression_result_none();
		res.type = leftRes.type;
		res.value = LLVMBuildSub(llvmData->builder, leftRes.value, rightRes.value, "sum");
		return res;
	}
	if (leftRes.type == vars->standard.literalFloatType ||
		leftRes.type == vars->standard.float32Type ||
...
static KarExpressionResult get_sign_mul(KarToken* token, KarLLVMData* llvmData, KarString* moduleName, KarVars* vars, KarProjectErrorList* errors) {
	...
	if (leftRes.type == vars->standard.decimalType ||
		...
		KarExpressionResult res = kar_expression_result_none();
		res.type = leftRes.type;
		res.value = LLVMBuildMul(llvmData->builder, leftRes.value, rightRes.value, "sum");
		return res;
	}
	if (leftRes.type == vars->standard.literalFloatType ||
		leftRes.type == vars->standard.float32Type ||
...
static KarExpressionResult get_sign_mod_clean(KarToken* token, KarLLVMData* llvmData, KarString* moduleName, KarVars* vars, KarProjectErrorList* errors) {
	...
	if (leftRes.type == vars->standard.decimalType ||
		...
		LLVMPositionBuilderAtEnd(llvmData->builder, mergeBlock);
		LLVMValueRef phi = LLVMBuildPhi(llvmData->builder, LLVMTypeOf(res.value), "ph_div");
		LLVMAddIncoming(phi, &zero, &thenBlock, 1);
		LLVMAddIncoming(phi, &res.value, &elseBlock, 1);
...
static KarExpressionResult get_sign_bit_right(KarToken* token, KarLLVMData* llvmData, KarString* moduleName, KarVars* vars, KarProjectErrorList* errors) {
	...
	if (leftRes.type == vars->standard.decimalType ||
		...
		KarExpressionResult res = kar_expression_result_none();
		res.type = leftRes.type;
		res.value = LLVMBuildAShr(llvmData->builder, leftRes.value, rightRes.value, "var");
		return res;
	}
	...
	if (leftRes.type == vars->standard.hexadecimalType ||
		...
		KarExpressionResult res = kar_expression_result_none();
		res.type = leftRes.type;
		res.value = LLVMBuildLShr(llvmData->builder, leftRes.value, rightRes.value, "var");
		return res;
	}
...
static KarExpressionResult get_sign_bit_left(KarToken* token, KarLLVMData* llvmData, KarString* moduleName, KarVars* vars, KarProjectErrorList* errors) {
	...
	if (leftRes.type == vars->standard.decimalType ||
		...
		KarExpressionResult res = kar_expression_result_none();
		res.type = leftRes.type;
		res.value = LLVMBuildShl(llvmData->builder, leftRes.value, rightRes.value, "var");
		return res;
	}
...
static KarExpressionResult get_sign_equal(KarToken* token, KarLLVMData* llvmData, KarString* moduleName, KarVars* vars, KarProjectErrorList* errors) {
	...
	if (leftRes.type == vars->standard.boolType ||
		...
		res.value = LLVMBuildICmp(llvmData->builder, LLVMIntEQ, leftRes.value, rightRes.value, "var");
		return res;
	}
	...
	if (leftRes.type == vars->standard.literalFloatType ||
		...
		KarExpressionResult res = kar_expression_result_none();
		res.type = vars->standard.boolType;
		res.value = LLVMBuildFCmp(llvmData->builder, LLVMRealOEQ, leftRes.value, rightRes.value, "var");
		return res;
	}
	...
	if (leftRes.type == vars->standard.stringType) {
		...
		res.type = vars->standard.boolType;
		LLVMValueRef in[] = {leftRes.value, rightRes.value};
		res.value = LLVMBuildCall(llvmData->builder, llvmData->isEqualString, in, 2, "isEqualString");
		return res;
	}
...
static KarExpressionResult get_sign_greater(KarToken* token, KarLLVMData* llvmData, KarString* moduleName, KarVars* vars, KarProjectErrorList* errors) {
	...
	if (leftRes.type == vars->standard.decimalType ||
		...
		KarExpressionResult res = kar_expression_result_none();
		res.type = vars->standard.boolType;
		res.value = LLVMBuildICmp(llvmData->builder, LLVMIntSGT, leftRes.value, rightRes.value, "var");
		return res;
	}
	...
	if (leftRes.type == vars->standard.hexadecimalType ||
		...
		KarExpressionResult res = kar_expression_result_none();
		res.type = vars->standard.boolType;
		res.value = LLVMBuildICmp(llvmData->builder, LLVMIntUGT, leftRes.value, rightRes.value, "var");
		return res;
	}
	...
	if (leftRes.type == vars->standard.literalFloatType ||
		...
		KarExpressionResult res = kar_expression_result_none();
		res.type = vars->standard.boolType;
		res.value = LLVMBuildFCmp(llvmData->builder, LLVMRealOGT, leftRes.value, rightRes.value, "var");
		return res;
	}
	...
	if (leftRes.type == vars->standard.stringType) {
		...
		res.type = vars->standard.boolType;
		LLVMValueRef in[] = {leftRes.value, rightRes.value};
		res.value = LLVMBuildCall(llvmData->builder, llvmData->isGreaterString, in, 2, "isEqualString");
		return res;
	}
...
static KarExpressionResult get_sign_greater_or_equal(KarToken* token, KarLLVMData* llvmData, KarString* moduleName, KarVars* vars, KarProjectErrorList* errors) {
	...
	if (leftRes.type == vars->standard.decimalType ||
		...
		KarExpressionResult res = kar_expression_result_none();
		res.type = vars->standard.boolType;
		res.value = LLVMBuildICmp(llvmData->builder, LLVMIntSGE, leftRes.value, rightRes.value, "var");
		return res;
	}
	...
	if (leftRes.type == vars->standard.hexadecimalType ||
		...
		KarExpressionResult res = kar_expression_result_none();
		res.type = vars->standard.boolType;
		res.value = LLVMBuildICmp(llvmData->builder, LLVMIntUGE, leftRes.value, rightRes.value, "var");
		return res;
	}
	...
	if (leftRes.type == vars->standard.literalFloatType ||
		...
		KarExpressionResult res = kar_expression_result_none();
		res.type = vars->standard.boolType;
		res.value = LLVMBuildFCmp(llvmData->builder, LLVMRealOGE, leftRes.value, rightRes.value, "var");
		return res;
	}
	...
	if (leftRes.type == vars->standard.stringType) {
		...
		res.type = vars->standard.boolType;
		LLVMValueRef in[] = {leftRes.value, rightRes.value};
		res.value = LLVMBuildCall(llvmData->builder, llvmData->isGreaterOrEqualString, in, 2, "isEqualString");
		return res;
	}
...
static KarExpressionResult get_sign_less_or_equal(KarToken* token, KarLLVMData* llvmData, KarString* moduleName, KarVars* vars, KarProjectErrorList* errors) {
	...
	if (leftRes.type == vars->standard.decimalType ||
		...
		KarExpressionResult res = kar_expression_result_none();
		res.type = vars->standard.boolType;
		res.value = LLVMBuildICmp(llvmData->builder, LLVMIntSLE, leftRes.value, rightRes.value, "var");
		return res;
	}
	...
	if (leftRes.type == vars->standard.hexadecimalType ||
		...
		KarExpressionResult res = kar_expression_result_none();
		res.type = vars->standard.boolType;
		res.value = LLVMBuildICmp(llvmData->builder, LLVMIntULE, leftRes.value, rightRes.value, "var");
		return res;
	}
	...
	if (leftRes.type == vars->standard.literalFloatType ||
		...
		KarExpressionResult res = kar_expression_result_none();
		res.type = vars->standard.boolType;
		res.value = LLVMBuildFCmp(llvmData->builder, LLVMRealOLE, leftRes.value, rightRes.value, "var");
		return res;
	}
	...
	if (leftRes.type == vars->standard.stringType) {
		...
		res.type = vars->standard.boolType;
		LLVMValueRef in[] = {leftRes.value, rightRes.value};
		res.value = LLVMBuildCall(llvmData->builder, llvmData->isLessOrEqualString, in, 2, "isEqualString");
		return res;
	}
