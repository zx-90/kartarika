...
KarParserStatus kar_parser_make_while(KarToken* token, KarProjectErrorList* errors) {
	...
	if (whilePos != 0) {
		kar_project_error_list_create_add(errors, &whileToken->cursor, 1, "Ключевое слово \"пока\" должно стоять в начале команды.");
		return KAR_PARSER_STATUS_ERROR;
	}
	
	if (kar_token_child_count(token) < 2) {
		// TODO: Ссылка на последний символ в токене.
		kar_project_error_list_create_add(errors, &whileToken->cursor, 1, "После ключевого слова \"пока\" ожидалось условие (выражение, возвращающее булево значение).");
		return KAR_PARSER_STATUS_ERROR;
	}
	KarToken* whileExpressionToken = kar_token_child_get(token, 1);
	if (!kar_parser_is_expression(whileExpressionToken->type)) {
		kar_project_error_list_create_add(errors, &whileExpressionToken->cursor, 1, "Здесь ожидалось выражение, возвращающее булево значение.");
		return KAR_PARSER_STATUS_ERROR;
	}
	
	if (kar_token_child_count(token) < 3) {
		// TODO: Ссылка на последний символ в токене.
		kar_project_error_list_create_add(errors, &whileExpressionToken->cursor, 1, "Здесь ожидался знак двоеточия.");
		return KAR_PARSER_STATUS_ERROR;
	}
	KarToken* colonToken = kar_token_child_get(token, 2);
	if (colonToken->type != KAR_TOKEN_SIGN_COLON) {
		kar_project_error_list_create_add(errors, &colonToken->cursor, 1, "Здесь ожидался знак двоеточия.");
		return KAR_PARSER_STATUS_ERROR;
	}
	
	if (kar_token_child_count(token) < 4) {
		// TODO: Ссылка на последний символ в токене.
		kar_project_error_list_create_add(errors, &colonToken->cursor, 1, "Не могу найти тело блока \"пока\".");
		return KAR_PARSER_STATUS_ERROR;
	}
	KarToken* bodyToken = kar_token_child_get(token, 3);
	if (bodyToken->type != KAR_TOKEN_BLOCK_BODY) {
		kar_project_error_list_create_add(errors, &bodyToken->cursor, 1, "Здесь ожидалось найти тело блока \"пока\".");
		return KAR_PARSER_STATUS_ERROR;
	}
	if (kar_token_child_count(token) > 4) {
		kar_project_error_list_create_add(errors, &kar_token_child_get(token, 4)->cursor, 1, "Найдены лишние элементы после тела блока.");
		return KAR_PARSER_STATUS_ERROR;
	}
