...
KarParserStatus kar_parser_make_break(KarToken* token, KarString* moduleName, KarProjectErrorList* errors) {
	...
	for (size_t i = 0; i < kar_token_child_count(token); ++i) {
		...
		if (child->type == KAR_TOKEN_COMMAND_CONTINUE) {
            ...
            kar_project_error_list_create_add(errors, moduleName, &nextChild->cursor, 1, "Лишнее выражение после ключевого слова \"следующий\".");
			return KAR_PARSER_STATUS_ERROR;
		}
        kar_project_error_list_create_add(errors, moduleName, &child->cursor, 1, "Здесь ожидалось ключевое слово \"прервать\" или \"следующий\".");
		return KAR_PARSER_STATUS_ERROR;
	}
