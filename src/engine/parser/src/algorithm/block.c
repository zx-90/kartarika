...
KarParserStatus kar_parser_make_block(KarToken* token, KarProjectErrorList* errors) {
	...
	if (blockPos != 0) {
		kar_project_error_list_create_add(errors, &kar_token_child_get(token, blockPos)->cursor, 1, "Ключевое слово \"блок\" должно стоять в начале команды.");
		return KAR_PARSER_STATUS_ERROR;
	}
	
	if (kar_token_child_count(token) < 2) {
		// TODO: Здесь должна быть последняя позиция в токене.
		kar_project_error_list_create_add(errors, &kar_token_child_get(token, blockPos)->cursor, 1, "Не возможно найти знак двоеточия после ключевого слова \"блок\".");
		return KAR_PARSER_STATUS_ERROR;
	}
	KarToken* colonToken = kar_token_child_get(token, 1);
	if (colonToken->type != KAR_TOKEN_SIGN_COLON) {
		kar_project_error_list_create_add(errors, &colonToken->cursor, 1, "Не возможно найти знак двоеточия после ключевого слова \"блок\".");
		return KAR_PARSER_STATUS_ERROR;
	}
	
	if (kar_token_child_count(token) < 3) {
		// TODO: Здесь должна быть последняя позиция в токене.
		kar_project_error_list_create_add(errors, &colonToken->cursor, 1, "Не возможно найти тело блока.");
		return KAR_PARSER_STATUS_ERROR;
	}
	KarToken* bodyToken = kar_token_child_get(token, 2);
	if (bodyToken->type != KAR_TOKEN_BLOCK_BODY) {
		kar_project_error_list_create_add(errors, &bodyToken->cursor, 1, "Не возможно найти тело блока.");
		return KAR_PARSER_STATUS_ERROR;
	}
	if (kar_token_child_count(token) > 3) {
		kar_project_error_list_create_add(errors, &kar_token_child_get(token, 3)->cursor, 1, "Найдены лишние элементы после тела блока.");
		return KAR_PARSER_STATUS_ERROR;
	}
