...
KarParserStatus kar_parser_make_if(KarToken* parent, size_t commandNum, KarProjectErrorList* errors) {
	...
	if (ifPos != 0) {
		kar_project_error_list_create_add(errors, &ifToken->cursor, 1, "Ключевое слово \"если\" должно стоять в начале команды.");
		return KAR_PARSER_STATUS_ERROR;
	}
	
	if (kar_token_child_count(token) < 2) {
		// TODO: Ссылка на последний символ в токене.
		kar_project_error_list_create_add(errors, &ifToken->cursor, 1, "После ключевого слова \"если\" ожидалось условие (выражение, возвращающее булево значение).");
		return KAR_PARSER_STATUS_ERROR;
	}
	KarToken* ifExpressionToken = kar_token_child_get(token, 1);
	if (!kar_parser_is_expression(ifExpressionToken->type)) {
		kar_project_error_list_create_add(errors, &ifExpressionToken->cursor, 1, "Здесь ожидалось выражение, возвращающее булево значение.");
		return KAR_PARSER_STATUS_ERROR;
	}
	
	if (kar_token_child_count(token) < 3) {
		// TODO: Ссылка на последний символ в токене.
		kar_project_error_list_create_add(errors, &ifExpressionToken->cursor, 1, "Здесь ожидался знак двоеточия.");
		return KAR_PARSER_STATUS_ERROR;
	}
	...
	if (kar_token_child_count(token) < 4) {
		// TODO: Ссылка на последний символ в токене.
		kar_project_error_list_create_add(errors, &colonToken->cursor, 1, "Не могу найти тело блока \"если\".");
		return KAR_PARSER_STATUS_ERROR;
	}
	KarToken* bodyToken = kar_token_child_get(token, 3);
	if (bodyToken->type != KAR_TOKEN_BLOCK_BODY) {
		kar_project_error_list_create_add(errors, &bodyToken->cursor, 1, "Здесь ожидалось найти тело блока \"если\".");
		return KAR_PARSER_STATUS_ERROR;
	}
	if (kar_token_child_count(token) > 4) {
