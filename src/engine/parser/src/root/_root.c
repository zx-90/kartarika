...
static bool parse_line(KarToken* child, KarProjectErrorList* errors) {
	...
	kar_project_error_list_create_add(errors, &child->cursor, 1, "Неизвестный корневой элемент. Необходимо объявление поля, константы или метода");
	return false;
}
