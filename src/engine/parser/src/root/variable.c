...
static bool parse_modifiers(KarToken* token, size_t variable, KarProjectErrorList* errors)
{
	...
	for (size_t i = 0; i < kar_token_child_count(modifiers); i++) {
		...
		if (
			...
			kar_project_error_list_create_add(errors, &child->cursor, 1, "Некорректный модификатор при объявлении поля.");
			return false;
		}
		if (
			child->type == KAR_TOKEN_MODIFIER_STAT ||
			...
			if (static_modifier) {
				kar_project_error_list_create_add(errors, &child->cursor, 1, "Объявлено более одного модификатора статичности при инициализации переменной.");
				return false;
			} else {
		...
		if (
			...
			if (inherit_modifier) {
				kar_project_error_list_create_add(errors, &child->cursor, 1, "Объявлено более одного модификатора области видимости при инициализации переменной.");
				return false;
			} else {
...
static bool parse_const_name(KarToken* token, KarProjectErrorList* errors) {
	const size_t CHILD_INDEX = 1;
	if (CHILD_INDEX == kar_token_child_count(token)) {
		kar_project_error_list_create_add(errors, &kar_token_child_get(token, CHILD_INDEX)->cursor, 1, "Отсутствует имя переменной.");
		return false;
	}
	if (kar_token_child_get(token, CHILD_INDEX)->type != KAR_TOKEN_IDENTIFIER || kar_token_child_count(kar_token_child_get(token, CHILD_INDEX)) > 0) {
		kar_project_error_list_create_add(errors, &kar_token_child_get(token, CHILD_INDEX)->cursor, 1, "Имя переменной должно быть идентификатором.");
		return false;
	}
...
static bool parse_var_name(KarToken* token, KarProjectErrorList* errors) {
	const size_t CHILD_INDEX = 1;
	if (CHILD_INDEX == kar_token_child_count(token)) {
		kar_project_error_list_create_add(errors, &kar_token_child_get(token, CHILD_INDEX)->cursor, 1, "Отсутствует имя переменной.");
		return false;
...
static bool parse_equation_sign(KarToken* token, KarProjectErrorList* errors) {
	size_t CHILD_INDEX = 1;
	if (CHILD_INDEX == kar_token_child_count(token)) {
		kar_project_error_list_create_add(errors, &kar_token_child_get(token, CHILD_INDEX)->cursor, 1, "Неожиданный конец присвоения. Переменная должны быть инициализирована.");
		return false;
	}
	if (kar_token_child_get(token, CHILD_INDEX)->type != KAR_TOKEN_SIGN_ASSIGN) {
		kar_project_error_list_create_add(errors, &kar_token_child_get(token, CHILD_INDEX)->cursor, 1, "Переменная должна быть инициализирована.");
		return false;
	}
	kar_token_child_erase(token, CHILD_INDEX);
	return true;
...
static bool parse_expression(KarToken* token, KarProjectErrorList* errors) {
	size_t CHILD_INDEX = 1;
	if (CHILD_INDEX == kar_token_child_count(token)) {
		// TODO: Надо положение курсора в конце строки вычислять.
		kar_project_error_list_create_add(errors, &kar_token_child_get(token, CHILD_INDEX - 1)->cursor, 1, "Неожиданный конец присвоения. Отсутствует правая часть присвоения.");
		return false;
	}
	if (kar_token_child_count(token) > CHILD_INDEX + 1) {
		kar_project_error_list_create_add(errors, &kar_token_child_get(token, CHILD_INDEX + 1)->cursor, 1, "Слишком много выражений в правой части присвоения.");
		return false;
	}
	if (!kar_parser_is_expression(kar_token_child_get(token, CHILD_INDEX)->type)) {
		kar_project_error_list_create_add(errors, &kar_token_child_get(token, CHILD_INDEX)->cursor, 1, "Некорректное выражение в правой части присвоения.");
		return false;
	}
...
KarParserStatus kar_parser_make_constant(KarToken* token, KarProjectErrorList* errors)
{
...
KarParserStatus kar_parser_make_variable(KarToken* token, KarProjectErrorList* errors)
{
