...
// TODO: Некоторые функции C заисят от компилятора. В основном это связано с разной разрядностью
//       типов float/double/long double, char/short/int/long int/ long long.
//       Кроме того, функции могут быть неоптимальны для Картарики, поэтому
//       необходимо посмотреть, какие функции необходимо переписать.
