...
стат метод Запустить(Массив(Строка) параметры) Число:
...
Пример преобразования типа число в тип число8:
...
Из плавающих в целые/счетные преобразуется с неопределенностью, так как у чисел с плавающей точкой есть состояние "не число". (??? Как обрабатывать слишком большие числа? Например 257.3 в Число8. Обрезать до нуля? И вообще округлять или к меньшему целому преобразовывать?)
