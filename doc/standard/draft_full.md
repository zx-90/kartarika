...
Из плавающих в целые/счетные преобразуется с неопределенностью, так как у чисел с плавающей точкой есть состояние "не число". (??? Как обрабатывать слишком большие числа? Например 257.3 в Целое8. Обрезать до нуля? И вообще округлять или к меньшему целому преобразовывать?)
...
### 6.2. Операция скобки (операция изменения приоритетов).

Операция скобок меняет приоритет выполнения операций.

### 6.3. Операция выбора элемента.
...
Можно добавить оператор ".?" (или "?."), который будет обращаться к неопределнному полю класса и возвращать либо значение поля либо пусто. Необходимо также иметь возможность несколько раз использовать такой оператор при обращении. Например `Воин.?Магия.?ЧёрнаяМагия` - То есть, если у воина есть достпу к магии и к черной магии, то возвращает её неопределённый тип. Если на каком-то этапе Магия или ЧёрнаяМагия имеют значение Пусто, то возвчращается значение Пусто типа неопределенность ЧёрнаяМагия.

### 6.4. Операция обращения к методу.
...
Надо будет добавить арифметические операции с неопределённостями.
