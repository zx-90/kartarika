...
Оператор неопределенность применяется к типу. В результате получается неопределённый тип. Неопределённый тип может принимать те же значения, что и определённый тип, но имеет дополнительное значение Пусто. Если начальное значение не определено, то по умолчанию принимается значение Пусто.

Пример:

~~~~
поле к1 = Целое? // Аналогично к1 = Целое?(Пусто)
поле к2 = Целое?(Пусто)
поле к3 = Целое?(4)
~~~~
