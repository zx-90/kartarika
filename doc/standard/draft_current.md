...
Подробное описание синтаксиса модулей (см. ???).
...
Полное описание всех доступных функций и переменных смотри в разделе стандартной библиотеки (см. ???)
...
В начале строки Пробел выполняет также дополнительную функцию определения к какому блоку принадлежит строка (см описание блоков ???). Один символ горизонтальной табуляции является табуляцией длиной в четыре пробела, комментарий - пробельный символ длиной в ноль пробелов.
...
С учётом вышеописанных условий, грамматическое описание идентификатора будет выглядет так:
...
#### 2.7.2. Цифра в шестнадцатиричном формате.
...
ШестнадцатиричноеЧисло =
    "0ш" ШестнадцатиричнаяЦифра{ШестнадцатиричнаяЦифра}
~~~~
...
ИсключающийСимволЮникод =
    СимволИсключения ш
    ШестнадцатиричнаяЦифра{ШестнадцатиричнаяЦифра}
    [СимволПробела]
...
Также операторами являются следующие ключевые слова, определённые в (??? см.)
...
Подробное описание значений этих токенов описаны в главе выражения (??? ссылка).
...
После раделения модуля на строки, пустые строки игнорируются. Пустые строки это строки модуля, которые состоят только из пробельных символов (то есть символов пробела, табуляции и комментариев). Формально пустую строку можно описать следующим образом:
...
Если у заголовка блока размер отступа равен 0 символов, это значит что он не имеет родительского блока и называется корневым блоком или корневым элементом модуля. Описание корневых блоков находится в главе (см ???). Описание некорневых блоков можно найти в главе Алгоритмы (см ???).
...
Перевод из неопределенного типа в соответствующий ему определенный можно совершить с помощью помощью блока "раскрыть" (??? ссылка). Пример:
...
~~~~
Выражение =
    Идентификатор | ЦелоеЧисло | ШестнадцатиричноеЧисло |
...
- если входным типом является ШестнадцатиричноеЧисло, то выходной - Счётное.
...
В пункте 5.1 (см ???) написано, что литерал ЦелоеЧисло явлется типом Целое (что равносильно типу Целое32), а литерал ШестнадцатиричноеЧисло является типом Счётное (что равносильно типу Счётное32).
...
Однако для отрицательных чисел существует несколько альтернативных спобов вычисления q и r. В нашем случае для отрицательных чисел введём дополнительное правило:
...
В данной главе используется 2 вида описания: более строгое грамматическое описание и упрощенное синтаксическое, для лучшего понимани.
...
* стат - статическое. Одно поле, создаваемое для всех экзепляров класса.
...
Заголовок метода состояит из следующих друг за другом частей:
...
После заголовка, со следующей строки разполагается тело метода (алгоритм метода).
...
* динамический - не статический метод. Метод может работать с динаическими полями и методами класса. (значение по-умолчанию)
...
* перегруженный - обязательный модификтор, если метод перегружает родительский. В этом случае он также становится финализируемым по-умолчанию. Необходимо добавить в родительский метод модификатор "наследуемый", если от него будут наследоваться.
