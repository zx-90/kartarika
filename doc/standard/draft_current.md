...
КлючевоеСлово =
    ...
    АлгоритмРаскрыть | АлгоритмЕсли | АлгоритмИначе |
    АлгоритмПовторить | АлгоритмПока | АлгоритмСледующий |
    АлгоритмПрервать | ОбъявлениеПоля | ОбъявлениеКонстанты |
...
Когда пустые строки удалены, все осатльные строки объединяются в блоки по иерархической структуре.
...
ЗаголовокБлока = ОтступВерхнегоУровня <Описание блока> : НоваяСтрока
СтрокаБлока = ОтступВерхнегоУровня ОтступБлока команда НоваяСтрока

Блок = ЗаголовокБлока СтрокаБлока {СтрокаБлока}
~~~~
...
~~~~
СтрокаПустогоБлока = ОтступВерхнегоУровня ОтступБлока АлгоритмПропустить НоваяСтрока

ПустойБлок = ЗаголовокБлока СтрокаПустогоБлока
~~~~
...
#### 5.31.1. Целочисленное деление и остаток для отрицательных чисел.
...
#### 5.31.2. Вычисление логических операций.
...
Можно прервать не только текущий цикл, но и стоящие выше. Для этого надо написать ключевое слово "прервать" столько раз, из скольки циклов вы хотите выйти. И даже добавить после этого ключевое слово "следующий". Но ключевое слово "следующий" можно добавить только 1 раз.
