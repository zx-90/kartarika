...
~~~~
[Модификаторы] функция [ИмяФункции] ([ВходныеДанные]) [ВыходнойТип]:
~~~~

Сначала следуют модификаторы, затем ключевое слово "функция" (??? возможно можно удалить), затем в скобках перечисление входных переменных или простые скобки без входных данных, затем тип возвращаемого значения, затем ставится двоеточие.
...
~~~~
функция Обработать(число а, строка с, буль б) буль:
~~~~
...
~~~~
функция Сумма(число а, число б) число:
    вернуть а + б
~~~~
...
~~~~
функция Проверить(число а) буль:
    если а меньше 0:
        результат = нет
    иначе:
        результат = да
