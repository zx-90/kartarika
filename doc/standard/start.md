...
(??? добавить список ключевых слов из последующих глав: числа, поля, пусто, модификаторы полей и функций, модификаторы для калсса и т.д. и т.п.)
...
Шестнадцатиричное число начинается с символов 0ш а затем идут одна или несколько шестнадцатиричных цифр: от нуля до девяти и далее A=10, Б=11, В=12, Г=13, Д=14, Е=15.
...
(??? Возможно стоит добавить юникод символы. Что-то типа \юНННН).
...
Корневым блоком может быть описанием функции (??? ссылка), юнит-тестом (??? ссылка), описанием импорта (??? ссылка). (??? возможно что-то ещё добавить).
...
Перевод из неопределенного типа в соответствующий ему определенный можно совершить с помощью вызова функции "или" (??? ссылка на подробное описание) или с помощью блока "раскрыть" (??? ссылка на подробное описание). Пример:
...
Структура - это набор нескольких полей. В отличие от класса не могут иметь функций и все поля открыты. (??? Нужен ли такой тип в программе? Фактически это просто обрезанный класс.)
...
* скрытое - видимо только внутри класса (значение по-умолчанию)
* наследуемое - видимо внутри класса и его наследников.
* местное - видимо внутри класса, его наследников и всех классов в текущем каталоге, но не наследуемых от местных классов (??? обсуждаемо)
* открытое - видимо всем.

Модификаторы финализации:
* финал - финализированное (значение по-умолчанию)
* наследуемое - не финализированное.
...
??? Возможно, нужно сделать аналоги в виде вызова функций для использования в шаблонах.
...
* би - побитовое и
* били - побитовое или
* биили - побитовое исключающее или
* бне - побитовое не
...
* установить - установить бит
* снять - снять бит
* инвертировать - инвертировать бит.

а установить 3 // установить 3й бит в переменной а.

Левая и правая часть разных типов. Возвращает тот же тип, что и переменная слева.
...
### 6.6. Приоритет операций.

Круглые скобки - оператор изменения приоретита операций.

(??? Разписать приоритеты)
...
~~~~
[Модификаторы] функция [ИмяФункции] ([ВходныеДанные]) возвращает [ВыходнойТип]:
~~~~

Сначала следуют модификаторы, затем ключевое слово "функция" (??? возможно можно удалить), затем в скобках перечисление входных переменных или прастые скобки без входных данных, затем ключевое слово "возвращает" (??? возможно сократить до "в" или убрать), затем тип возвращаемого значения, затем ставится двоеточие.

Функция может не возвращать никаких данных. В этом случае не надо писать ключевое слово "возвращает" и ВыходнойТип

### 7.1. Модификаторы функции.

Модификаторы статичности:

* стат - статическая
* нестат - не статическая функция (значение по-умолчанию)

Модификаторы области видимости:

* скрытая - видима только внутри класса (значение по-умолчанию)
* наследуемая - видима внутри класса и его наследников.
* местная - видима внутри класса, его наследников и всех классов в текущем каталоге, но не наследуемых от местных классов (??? обсуждаемо)
* открытая - видима всем.

Модификаторы финализации:

* финал - финализированное (значение по-умолчанию)
* наследуемая - не финализирована. (Сразу же становится динамической функцией.)

Модификатор перегруженности функции:

* перегруженная - обязательный модификтор, если функция перекрывает родительскую. В этом случае она также становится финализируемой по-умолчанию. Необходимо добавить "наследуемая", если от неё будут наследоваться.

### 7.2. Типы передаваемых данных.

Входные данные функции перечисляются через запятую, где сначала пишется тип переменной, а затем имя переменной:

~~~~
функция Обработать(число а, строка с, буль б) возвращает буль
~~~~

Передаваемые данные являются константными. То есть нельзя изменять эти переменные и поля в классе. Можно вызывать только константные функции. Константными функциями называются те функции класса, которые не изменяются поля класса. Эти константные функции вычисляются компилятором (??? сложно ли это будет сделать).

(??? Возможно для не ссылочных типов разрешить модификацию внутри функции, но изменения не влияют на переменные вне функции).

~~~~
функция Сумма(число а, число б) возвращает число:
    вернуть а + б
~~~~

Если переменную надо изменить, то перед именем параметра функции ставится знак *. Например:
...
При вызове такой функции перед изменяемой переменной также ставится знак *. Пример:
...
Возвращаемое значение может присутствовать или отсутствовать. В случае присутствия возвращаемого значения, оно имеет имя "результат", с которым можно работать в функции. (??? следить, чтобы перед работой с результатом ему должно буть присвоено значение. Сложно ли будет реализовать?)

~~~~
функция Проверить(число а) возвращает буль:
    если а меньше 0:
        результат = нет
    иначе:
        результат = да
...
Интерфейс обхода. Класс должен иметь следующие функции:
...
Тест фактически представляет собой статическую функцию без входных параметров, на выходе выдаёт переменную типа буль (??? Можно на выходе выдавать строку сообщения об ошибке. Пустая строка - всё хорошо, если есть строка - то это сообщение об ошибке). Тесты нельзя вызывать из других функций и из других тестов.

Внутри тесты можно проверять поля, и если проверка не прошла, то выходит из функции с сообщением об ошибке.

функции, изпользуемые внутри теста:
~~~~
тест(выражение)
тестНет(выражение)
...
??? Набор функций обсуждаем

??? Как тесты запускать из командной строки. Билдинг и запуск.

## 10. Шаблоны.

В программе можно создать шаблон класса и шаблон функции.

### 10.1. Шаблоны функции.

Синтаксис шаблон функции:

~~~~
[атрибуты] шаблон [имяФункции] ([параметры Шаблона])([параметры функции]) возвращает:
    [тело функции]
~~~~
...
Возможно даже просто перечислять параметры шаблона как параметры функции. Надо подумать над этим.
...
видимая функция Добавить(Т слагаемое):
    сумма = сумма.Сложить(слагаемое)

видимая функция Сумма() возвращает Т:
    вернуть сумма
...
### 10.3. Функции с переменным количеством параметров.

Функции могут быть с переменным количеством параметров. (??? Это шаблон вообще? Как реализуется?)

Пример:

~~~~
функция Печатать(число):
    [телоФункции]

функция Печатать(строка):
    [телоФункции]

шаблон функции Печатать(тип Т)(Т поле, ...):
    Печатать(Т)
    Печатать(аргументы)
~~~~
...
Так как у нас нет циклических зависимостей в программе, то возможен цикл по полям типа Т. Возможно зайти в каждое поле и выполнить над ним цикл функции.
...
Подставляется перед полем или функцией. Пример:
...
Класс состоит из заголовка, блока импорта, статических и динамических полей а также статических и динамических функций и тестов.
...
Если заголовок не написан, то класс не является наследников ни от какого класса (??? второй вариант - все классы являются наследниками от класса объект. Хороший вариант, так как можно туда добавить некоторые процедуры и функции типа имени класса, возможности узнать, является ли он производным от другого класса и т.п.)
...
Тоже касается и динамических полей при инициализации экземпляра класса (??? Надо продумать варианты как сделать так, чтобы в конструкторе всегда инициализировались все динамические поля. Можно ли такое проверить на этапе компиляции. Все команды если циклов и входы в другие функции?).

Все функции в классе по-умолчанию финализированные. Если необходимо изменить поведение функции в наследнике, то в функции базового класса необбходимо написать соответствующее ключевое слово, разрешающее наследование, а в наследуемая функция должна иметь те же атрибуты, что и базовая функции. Заметьте, что нельзя, например, изменить область видимости функции в наследуемом классе.

Таким образом ООП дается достаточно ограниченно, возможна наследование только от одного класса и перегрузка функций. Никаких множественных наследований, интерфейсов, изменений области видимости.
...
Связывание с библиотекой и вызов функций происходит по тем же правилам, что и в extern "C" в языках C/C++. (??? Достаточно ли будет такого типа связывания? или надо что-то добавлять?) (??? Расписать, что сие конкретно означает?)
...
В первой строке пишется название библиотеки, далее перечисляются функции через ключевое слово экспорт.

Любой функции можно дать перевод через символ "/". В этом случае в программе можно использовать уже переведенное имя.
...
(??? Тут в примере надо подумать, можно ли передавать строки более элегантно. Это касается и строк, и массивов, и функций, и других объектов.)
