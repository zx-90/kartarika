...
Операция выбора элемента обозначается через символ "." и необходим для выбора поля, функции или другого элемента класса.
(??? что такое другой элемент? Тесты можно вызывать?)

### 6.3. Операция перчисления.

Используется в функции или в шаблонах функции. Как в объявлении так и при вызове. Перечисление выражений через запятую:

~~~~
ВызовФункции(поле1, поле2, поле3)
~~~~
...
Сначала следуют модификаторы, затем ключевое слово "функция" (??? возможно можно удалить), затем в скобках перечисление входных переменных или простые скобки без входных данных, затем ключевое слово "возвращает" (??? возможно сократить до "в" или убрать), затем тип возвращаемого значения, затем ставится двоеточие.
